...
CUSTOM_HEADERS = {
    "Origin": "https://buffsports.io",
    "Referer": "https://buffsports.io/",
...
def get_all_matches():
    ...
    for ep in endpoints:
        try:
            ...
            res = requests.get(f"https://buffsports.io/{ep}", timeout=10)
...
def get_embed_urls_from_api(source):
    try:
        ...
        if not s_name or not s_id:
            return []
        res = requests.get(f"https://buffsports.io/{s_name}/{s_id}", timeout=6)
        res.raise_for_status()
        data = res.json()
